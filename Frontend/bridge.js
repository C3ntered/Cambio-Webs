...
const getApiBase = () => {
  ...
  if (
    window.location.hostname === "cambiogame.com" ||
...
async function joinGame(username, roomId = null) {
  ...
  if (roomId) {
    ...
    const redKingVariant =
      document.getElementById("red-king-variant")?.checked || false;
...
function handleSocketMessage(event) {
  ...
  switch (message.type) {
    case "game_state":
      ...
      renderBoard(
        message.data.room,
        message.data.your_player_id || playerContext.playerId,
      );
      break;
    ...
    case "player_drew_card":
      if (message.data.player_id !== playerContext.playerId) {
        ...
        const name = p ? p.username : "Unknown";
        const source =
          message.data.source === "discard" ? "discard pile" : "deck";
    ...
    case "game_reset": {
      ...
      if (
        ...
        card1_index !== undefined &&
        player2_id !== undefined &&
        card2_index !== undefined
      ) {
    ...
    case "cards_swapped":
      ...
      const finishSwap = () => {
        ...
        if (
          !latestRoomState ||
          !latestRoomState.game_state ||
          message.data.room.game_state.turn_number >=
            latestRoomState.game_state.turn_number
        ) {
          latestRoomState = message.data.room;
        }
        ...
        if (player1_id !== undefined && card1_index !== undefined)
          highlightCard(player1_id, card1_index);
        if (player2_id !== undefined && card2_index !== undefined)
          highlightCard(player2_id, card2_index);
      };
    ...
    case "card_drawn":
      ...
      latestRoomState = message.data.room;
      notify(
        `You drew ${formatCard(message.data.card)}. Choose: swap with a hand card or discard.`,
      );
      renderBoard(message.data.room, playerContext.playerId);
      break;
    ...
    case "card_being_looked_at":
      ...
      const {
        player_id: looking_pid,
        target_player_id: t_pid,
        card_index: c_idx,
        duration: look_dur,
      } = message.data;
      ...
      // Apply immediately
      const lookedBtn = findCardElement(
        t_pid,
        c_idx,
        latestRoomState,
      ...
      const pName =
        latestRoomState.players.find((p) => p.player_id === looking_pid)
          ?.username || "Someone";
    ...
    case "ability_resolution":
      ...
      if (ability === "peek_self" || ability === "peek_other") {
        ...
        const targetName =
          latestRoomState?.players.find((p) => p.player_id === target_player_id)
            ?.username || "Unknown";
        notify(
          `You peeked at ${targetName}'s card #${card_index + 1}: ${formatCard(card)}`,
          dur,
        );
        ...
        if (!activeLookIndicators[first.player_id])
          activeLookIndicators[first.player_id] = {};
        activeLookIndicators[first.player_id][first.card_index] = "PERSIST";

        if (!activeLookIndicators[second.player_id])
          activeLookIndicators[second.player_id] = {};
        activeLookIndicators[second.player_id][second.card_index] = "PERSIST";

        renderBoard(latestRoomState, playerContext.playerId);

        const p1 = latestRoomState.players.find(
          (p) => p.player_id === first.player_id,
        );
        const p2 = latestRoomState.players.find(
          (p) => p.player_id === second.player_id,
        );
        notify(
          `Review cards: ${formatCard(first.card)} (${p1.username}) and ${formatCard(second.card)} (${p2.username})`,
          dur,
        );
      } else {
...
function callCambio() {
  if (
    confirm(
      "Are you sure you want to Call Cambio? This will end your turn and trigger the final round.",
    )
  ) {
...
function startAbilitySelection(ability) {
  ...
  if (panel && nameDisplay && desc) {
    ...
    if (ability === "peek_self")
      instructions = "Click one of YOUR cards to peek at it.";
    else if (ability === "peek_other")
      instructions = "Click one of an OPPONENT'S cards to peek at it.";
    else if (ability === "blind_swap")
      instructions = "Click ANY two cards to swap them.";
    else if (ability === "look_and_swap")
      instructions = "Click ANY two cards to look at them. (Swap optional)";
...
function renderBoard(room, yourPlayerId) {
  ...
  if (playerListContainer) {
    ...
    listTitle.style.color = "var(--modal-text)";
    ...
    room.players.forEach((player) => {
      ...
      item.style.backgroundColor =
        player.player_id === yourPlayerId
          ? "rgba(33, 150, 243, 0.2)"
          : "rgba(128, 128, 128, 0.1)";
      item.style.color = "var(--modal-text)";
      item.style.borderRadius = "4px";
      item.textContent =
        player.username + (player.player_id === yourPlayerId ? " (You)" : "");
  ...
  if (turnIndicator) {
    ...
    if (isWaiting) {
      ...
      turnIndicator.innerHTML =
        'Grace Period! Eliminate matching cards (<span id="grace-countdown">10</span>s) <br>';
      ...
      const currentPlayer = room.players.find(
        (p) => p.player_id === room.game_state.current_turn,
      );
      if (currentPlayer) {
  ...
  const isPlaying =
    room.status?.toLowerCase() === GAME_STATUS.PLAYING ||
    room.status?.toLowerCase() === GAME_STATUS.GRACE_PERIOD;
  const isViewingPhase = room.game_state?.viewing_phase;
  ...
  if (isPlaying) {
    if (discardPileContainer)
      discardPileContainer.style.display = isViewingPhase ? "none" : "block";
    if (deckPileContainer)
      deckPileContainer.style.display = isViewingPhase ? "none" : "block";
    if (cambioContainer)
      cambioContainer.style.display = isViewingPhase ? "none" : "block";
    if (myHandContainer) myHandContainer.style.display = "block";
    if (opponentsHandsContainer)
      opponentsHandsContainer.style.display = isViewingPhase ? "none" : "block";
    if (actionButtons)
      actionButtons.style.display = isViewingPhase ? "none" : "flex";
    ...
    if (abilityPanel) {
      if (pendingSwapDecision) {
        ...
        if (desc)
          desc.innerText = "Do you want to swap the cards you just saw?";
        if (controls) {
          controls.innerHTML = `
    ...
    if (callCambioBtn) {
      ...
      const canCall =
        room.game_state?.current_turn === yourPlayerId &&
        !pendingDrawnCard &&
        !pendingAbility &&
        !room.game_state.cambio_called;
      ...
      if (room.game_state.cambio_called) {
        ...
        callCambioBtn.title =
          "Cannot call Cambio after drawing or during ability";
      } else {
        callCambioBtn.title =
          "Call Cambio to end your turn and start final round";
      }
    }
      ...
      if (me) {
        ...
        me.hand.forEach((card, index) => {
          ...
          if (!card) {
            ...
            const isVisible =
              (isViewingPhase && isBottomCard) ||
              adminMode ||
              (activeLookIndicators[yourPlayerId] &&
                activeLookIndicators[yourPlayerId][index] === "PERSIST");
            ...
            if (isVisible) {
              ...
              btn.classList.add("card-back");
              btn.title = isViewingPhase
                ? isBottomCard
                  ? "Memorize this card!"
                  : "Face down"
                : isAwaitingDrawChoice
                  ? `Click to swap with drawn card`
                  : `Card #${index + 1}`;
            }
          }
    ...
    if (opponentsContainer && !isViewingPhase) {
      ...
      const opponents = room.players.filter(
        (p) => p.player_id !== yourPlayerId,
      );
      const totalOpponents = opponents.length;

      opponents.forEach((player, oppIndex) => {
        ...
        if (totalOpponents === 1) {
          ...
          const angle = Math.PI - (oppIndex / (totalOpponents - 1)) * Math.PI;
          leftPercent = 50 + Math.cos(angle) * 40;
          topPercent = 45 - Math.sin(angle) * 35;
        }
        ...
        cardsDiv.style.gridTemplateColumns = "repeat(2, 1fr)";
        cardsDiv.style.gap = "8px";

        player.hand.forEach((_, index) => {
          const btn = document.createElement("button");
          btn.setAttribute("data-index", index);
          btn.classList.add("game-card", "card-back");
          ...
          if (eliminationTarget) {
            ...
            btn.addEventListener("click", () =>
              callEliminate(player.player_id, index),
            );
          } else {
            btn.style.cursor = "default";
            btn.disabled = true;
  ...
  if (discardPile) {
    discardPile.onclick = () => {
      if (latestRoomState) {
        const me = latestRoomState.players.find(
          (p) => p.player_id === playerContext.playerId,
        );
...
function copyRoomId() {
  ...
  const showCopiedFeedback = () => {
    ...
    if (button) {
      const originalText =
        button.getAttribute("data-original-text") || button.innerText;
...
function findCardElement(pid, idx, roomState, myPlayerId) {
  ...
  if (pid === myPlayerId) {
    ...
    if (oppContainer) {
      ...
      const opponents = roomState.players.filter(
      ...
      if (oppIndex !== -1 && oppContainer.children[oppIndex]) {
        container =
          oppContainer.children[oppIndex].querySelector(".opponent-cards");
      }
    }
  }
  ...
  if (container) {
    ...
    if (!btn)
      console.warn(
        "findCardElement: Button with data-index",
        idx,
...
function animateSwap(
  ...
  const el1 = findCardElement(
    player1_id,
    card1_index,
    latestRoomState,
    playerContext.playerId,
  );
  const el2 = findCardElement(
    player2_id,
    card2_index,
    latestRoomState,
    playerContext.playerId,
...
document.addEventListener("click", function (e) {
  const dropdown = document.getElementById("player-list-content");
